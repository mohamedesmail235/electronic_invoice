...
def create_qr_code(doc,method=None):
    ...
    invoice_datetime = doc.posting_date
    invoice_total = doc.grand_total
    vat_total = doc.total_taxes_and_charges
    # generate QR code
    png_bytes, base64_tlv = generate_zatca_qr_bytes(
        seller_name, vat_number, invoice_datetime, invoice_total, vat_total
    )

    file_name = f"zatca_qr_{attached_to_name}.png"

    # create File doc (private file)
    file_doc = save_file(
        fname=file_name,
        content=png_bytes,
        dt=attached_to_doctype,
        dn=attached_to_name,
        is_private=1,  # or 0 for public file
    )
...
def generate_zatca_qr_bytes(seller_name, vat_number, invoice_datetime, invoice_total, vat_total):
    """Generate ZATCA QR code and return PNG bytes + Base64 string."""
    def _tlv(tag, value):
    ...
    if isinstance(invoice_datetime, datetime):
        ...
        dt = datetime.combine(getdate(invoice_datetime), datetime.min.time())

    dt = dt.replace(tzinfo=timezone.utc)
    ts = dt.strftime("%Y-%m-%dT%H:%M:%SZ")
    for field in ['seller_name', 'vat_number', 'ts', 'invoice_total', 'vat_total']:
        if not locals()[field]:
    ...
    tlv_bytes = b''.join([
        ...
        _tlv(2, vat_number),
        _tlv(3, ts),
        _tlv(4, _fmt(invoice_total)),
        _tlv(5, _fmt(vat_total))
    ])
    b64 = base64.b64encode(tlv_bytes).decode('utf-8')
